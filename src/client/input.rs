...
//!
//! On Unix, reads stdin bytes and forwards framed input to the main event
//! loop. The server handles semantic parsing. On Windows, the default VTI
//! backend reads console `INPUT_RECORD`s directly; the crossterm fallback
//! may surface terminal control strings as character key events, so that
//! reader re-frames those control bytes before forwarding semantic client
//! input events.
...
/// Reads input from stdin and sends it to the main event loop.
///
/// This runs on a dedicated thread because stdin reading is blocking.
/// Unix forwards raw framed bytes; Windows forwards structured
/// `ClientInputEvent`s.
...
/// `ClientMessage::Input` to the server. The server handles semantic
/// parsing.
#[cfg(unix)]
...
#[cfg(any(windows, test))]
pub(super) fn windows_client_input_event_from_raw(
