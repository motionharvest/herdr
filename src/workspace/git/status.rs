...
use crate::workspace::{GitWorktreeState, WorkspaceGitStatusSnapshot};
...
pub(super) fn git_worktree_state(cwd: &Path) -> GitWorktreeState {
    ...
    if has_staged && has_unstaged {
        GitWorktreeState::Mixed
    } else if has_unstaged {
        GitWorktreeState::Unstaged
    } else if has_staged {
        GitWorktreeState::Staged
    } else {
...
mod tests {
    ...
    #[test]
    fn git_status_snapshot_reports_worktree_state_changes() {
        let root = temp_test_dir("worktree-state");
        run_git(&root, &["init"]);
        ...
        let (snapshot, cache) = git_status_snapshot_for_cwd(&root, None);
        assert_eq!(snapshot.worktree_state, GitWorktreeState::Clean);

        std::fs::write(root.join("tracked.txt"), "two\n").unwrap();
        let (snapshot, cache) = git_status_snapshot_for_cwd(&root, cache.as_ref());
        assert_eq!(snapshot.worktree_state, GitWorktreeState::Unstaged);

        run_git(&root, &["add", "tracked.txt"]);
        let (snapshot, cache) = git_status_snapshot_for_cwd(&root, cache.as_ref());
        assert_eq!(snapshot.worktree_state, GitWorktreeState::Staged);

        std::fs::write(root.join("tracked.txt"), "three\n").unwrap();
        let (snapshot, _) = git_status_snapshot_for_cwd(&root, cache.as_ref());
        assert_eq!(snapshot.worktree_state, GitWorktreeState::Mixed);
