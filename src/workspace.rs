...
impl Workspace {
    ...
    pub fn close_focused(&mut self) -> bool {
        ...
        if let Some((removed, _pane)) = self.active_tab_mut().and_then(Tab::close_focused) {
    ...
    pub fn remove_pane(&mut self, pane_id: PaneId) -> bool {
        ...
        if let Some((removed, _pane)) = self.tabs[tab_idx].remove_pane(pane_id) {
            self.unregister_pane(removed);
        }
        false
    }
    ...
    pub fn take_pane(&mut self, pane_id: PaneId) -> Option<(PaneId, PaneState)> {
        ...
        self.pane_git_statuses.remove(&detached.0);
        Some(detached)
    }
    ...
    pub(crate) fn git_status_for_pane(&self, pane_id: PaneId) -> WorkspaceGitStatusSnapshot {
        self.pane_git_statuses
            .get(&pane_id)
            .cloned()
            .unwrap_or_else(|| WorkspaceGitStatusSnapshot {
                branch: self.cached_git_branch.clone(),
    ...
    pub fn close_pane(&mut self, pane_id: PaneId) -> bool {
        ...
        if let Some((removed, _pane)) = self.tabs[tab_idx].close_pane(pane_id) {
            self.unregister_pane(removed);
        }
        false
    }
...
impl Workspace {
    ...
    pub(crate) fn test_split(&mut self, direction: Direction) -> PaneId {
        ...
        let new_id = tab
            .layout
            .split_focused_with_placement(direction, crate::layout::SplitPlacement::After);
        tab.panes
