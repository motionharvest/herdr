...
impl GhosttyPaneTerminal {
    ...
    pub fn encode_mouse_button(
        ...
        // An empty encoding means the terminal is not tracking this event, so
        // report it as unencoded rather than "forwarded as zero bytes".
        encoder
            .encode(&event)
            .ok()
...
mod tests {
    ...
    #[test]
    fn ghostty_mouse_button_encoding_is_none_without_mouse_reporting() {
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx).unwrap();
    ...
    #[test]
    fn ghostty_mouse_drag_encoding_is_none_without_motion_reporting() {
        let (tx, _rx) = mpsc::channel(4);
        let mut terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
        terminal.write(b"\x1b[?1000h\x1b[?1006h");
        let pane = GhosttyPaneTerminal::new(terminal, tx).unwrap();
