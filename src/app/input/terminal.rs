...
impl App {
    ...
    fn prepare_terminal_key_forward(&mut self, key: TerminalKey) -> Option<PreparedPaneInput> {
        ...
        if is_ctrl_q(key) {
            ...
            if self.state.confirm_detach_if_armed(now) {
...
mod tests {
    ...
    async fn terminal_direct_focus_pane_shortcut_switches_focus_without_leaving_terminal_mode() {
        ...
        app.handle_terminal_key(TerminalKey::new(KeyCode::Left, KeyModifiers::CONTROL))
            .await;
    ...
    #[tokio::test]
    async fn terminal_direct_ctrl_w_closes_focused_pane() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &Config::default(),
            true,
        ...
        app.state.view.pane_infos = app.state.workspaces[0]
            ...
            .layout
            .panes(Rect::new(0, 0, 80, 24));
        let pane_count_before = app.state.workspaces[0].layout.pane_count();

        app.handle_terminal_key(TerminalKey::new(KeyCode::Char('w'), KeyModifiers::CONTROL))
            .await;

        assert_eq!(
            app.state.workspaces[0].layout.pane_count(),
            pane_count_before - 1
        );
        assert_eq!(app.state.mode, Mode::Terminal);
    }
    ...
    #[tokio::test]
    async fn direct_detach_requires_two_ctrl_q_presses() {
        let mut app = app_for_mouse_test();
        app.state.keybinds.detach = crate::config::ActionKeybinds::direct("ctrl+q");

        app.handle_terminal_key(TerminalKey::new(KeyCode::Char('q'), KeyModifiers::CONTROL))
            .await;
    ...
    #[tokio::test]
    async fn first_ctrl_q_is_forwarded_to_focused_pane() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        ...
        let bytes = rx.try_recv().expect("first ctrl+q should reach pane");
        assert_eq!(bytes.as_ref(), &[0x11]);
        assert!(app.state.detach_confirm_armed(std::time::Instant::now()));
        assert!(rx.try_recv().is_err());
    }

    #[tokio::test]
    async fn second_ctrl_q_within_window_is_not_forwarded() {
        let mut app = app_for_mouse_test();
        let mut ws = Workspace::test_new("test");
        let pane_id = ws.tabs[0].root_pane;
        let pane_infos = ws.tabs[0].layout.panes(Rect::new(0, 0, 80, 24));
    ...
    #[tokio::test]
    async fn direct_detach_confirmation_clears_on_other_key() {
        let mut app = app_for_mouse_test();
        app.state.keybinds.detach = crate::config::ActionKeybinds::direct("ctrl+q");
