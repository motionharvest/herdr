...
fn action_for_key(
    ...
    for (bindings, action) in [
        ...
        (&kb.split_down, NavigateAction::SplitDown),
        (&kb.split_up, NavigateAction::SplitUp),
        (&kb.split_right, NavigateAction::SplitRight),
...
pub(super) fn execute_navigate_action_in_context(
    ...
    match action {
        ...
        NavigateAction::SplitLeft => {
            state.split_pane_with_placement(
                terminal_runtimes,
                Direction::Horizontal,
                crate::layout::SplitPlacement::Before,
            );
            leave_navigate_mode(state);
        }
        NavigateAction::SplitDown => {
            state.split_pane_with_placement(
                terminal_runtimes,
                Direction::Vertical,
...
"#,
    ...
    #[test]
    fn terminal_direct_close_pane_shortcut_maps_to_navigation_action() {
        let state = state_with_workspaces(&["test"]);

        let action = terminal_direct_navigation_action(
            &state,
            TerminalKey::new(KeyCode::Char('w'), KeyModifiers::CONTROL),
        );
...
"#,
    ...
    fn navigate_configured_q_detaches_in_persistence_mode() {
        ...
        state.keybinds.detach = crate::config::ActionKeybinds::prefix("q");
