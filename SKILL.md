...
**pane names** — every pane also has a stable human name, shown in the table and returned as `name` in `pane list` and `pane get`. an agent with a session title is named from that title: a headline that ends in a past verb becomes an `-er`/`-or` agentive on the last two content nouns (`naming-convention-explainer`), an imperative title uses the first content verb and the last two content words of its object (`yellow-banner-hider`, `playbill-link-adder`, `nav-links-adder`), a noun-only title becomes `-man` or `-ist` (`workman`, `machinist`), and anything else uses the first three words as a slug (`herdr-pane-title`), with `-2` on collision. a shell, or an agent that has not been titled yet, still gets a first name from the word list (`Olivia`, `Olivia-2` on collision). any command that takes a pane id also accepts the name, case-insensitive. names stay with their pane for the life of the terminal and survive restarts, so they are safer than compact ids across closes.
