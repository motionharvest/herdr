...
fn cross_area_agent_process_survives_detach_and_reattach() {
    ...
    // Reattach and ensure the client receives frames while API state retains the status.
    let mut client_b = UnixStream::connect(&client_socket).expect("client B should connect");
    client_handshake(&mut client_b, 12, 80, 24);
    assert!(wait_for_frame(&mut client_b, Duration::from_secs(5)));
    assert!(
        wait_for_agent_status(&api_socket, &pane_id, "working", Duration::from_secs(3)),
        "reattached client should not disturb persisted agent working status"
    );

    // Transition to idle and verify the persisted agent state changes.
...
fn cross_area_client_and_api_workspace_views_are_consistent() {
    ...
    // The attached client must receive a redraw after the API mutation, while
    // workspace identity is verified through the API now that desktop no longer
    // renders a persistent workspace sidebar.
