...
If the Herdr server stops and starts again, the original pane processes are gone. Herdr restores the saved session shape: workspaces, tabs, panes, cwd, layout, focus, each agent's done marker, and the composer Directory and agent you left showing.
...
Unsupported, invalid, duplicated, or stale session references restore as normal shells in the saved pane directory. An agent launched by Herdr with no reported session reference restarts from its original launch command so it remains available in the agent table, whether it was set down or open in a pane when the server stopped.
...
| Server restart | No | Yes | Only with pane screen history | With native agent session restore, or from the original command for agents launched by Herdr |
| Update without `--handoff` | Compatible servers keep running; restart-required servers may need stop/restart | Yes after restart | Only with pane screen history | With native agent session restore, or from the original command for agents launched by Herdr |
