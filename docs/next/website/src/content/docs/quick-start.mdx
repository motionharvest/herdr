...
Herdr is mouse-native. You can click panes, tabs, workspaces, and agents; drag borders; drag a pane name over another space to switch there and drop it into that layout, or onto `+ new` to fly it out; drag an agent from the table or sidebar onto `+ new` the same way; drag-select text to copy it to your clipboard; double-click a token to copy it directly; and use right-click menus. Copying does not require Ctrl+C.
