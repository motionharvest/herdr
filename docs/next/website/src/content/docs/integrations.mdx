---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Droid, Kimi Code CLI, OpenCode, Hermes Agent, Qoder CLI, and Grok.
---

Herdr detects supported agents automatically. Official integrations can add native session identity for restore, semantic state reports, or both. Claude Code, Codex, Droid, OpenCode, and Grok state comes from screen detection even when their session identity integrations are installed.
...
Claude Code, Codex, Droid, OpenCode, and Grok state detection is screen-read first. Their Herdr-owned integrations do not author `idle`, `working`, or `blocked` state. Pi, GitHub Copilot CLI, and Hermes Agent report semantic state and session identity. OMP, Kimi Code CLI, and Qoder CLI report semantic state but do not provide native session restore. Custom socket integrations can report state when they define state that is not visible in the native terminal UI.

Some integrations report native agent session references. Herdr uses official session references to resume Claude Code, Codex, GitHub Copilot CLI, Droid, Pi, Hermes Agent, OpenCode, and Grok panes after a Herdr server restart unless `[session] resume_agents_on_restore = false` disables it.

Native session restore requires current Herdr integrations: Pi integration version `2`, Claude Code version `5`, Codex version `5`, GitHub Copilot CLI version `1`, Droid version `1`, OpenCode version `4`, Hermes Agent version `2`, or Grok version `1`. OMP integration version `2`, Kimi Code CLI integration version `1`, and Qoder CLI integration version `1` report agent state only. Check installed versions with `herdr integration status`.
...
Herdr uses `~/.claude` by default, or `CLAUDE_CONFIG_DIR` when set. The Claude config directory must already exist. Install writes `hooks/herdr-agent-state.sh` and `hooks/herdr-statusline.sh`, and updates `settings.json` with Herdr hook entries and a `statusLine` command. The statusline script reports the Claude session's title to its pane; a statusline that was already configured is preserved in `hooks/herdr-statusline-delegate.json` and still renders the display. Uninstall removes the matching hook entries, deletes both scripts, and restores the previous `statusLine` configuration.
...
Install the Grok hook:

```bash
herdr integration install grok
```

The hook reports session identity through the same local socket API used by other integrations. Grok state comes from Herdr's screen detection. After the hook reports a session id, Herdr reads `chat_history.jsonl` for the typed prompt that fills Summary, and `summary.json` for the current model. The first prompt sticks until Grok sets the window title (OSC 0/2): that title is the same live text Grok writes to a host tab, activity plus session name, and Summary follows it as the turn proceeds. `Refresh Summary` on the row menu takes the latest prompt, or a sentence from a headless `grok -p` session when `[ui] refresh_summary_with_grok` is on. The column says `refreshing summary` until that session returns. Settings → experiments has a `prompt` field for the text sent to Grok (`[ui] refresh_summary_prompt`). `generated_title` fills Summary until a prompt has been parsed, if no window title has arrived yet.

Herdr uses `~/.grok` by default, or `GROK_HOME` when set. The Grok home directory must already exist. Install writes `hooks/herdr-agent-state.sh` and `hooks/herdr-agent-state.json`. Uninstall removes those two files and leaves any other hooks in that directory.
