...
The hook reports session identity through the same local socket API used by other integrations. Grok state comes from Herdr's screen detection. After the hook reports a session id, Herdr reads that session's `summary.json` for the generated title and the current model, which is what fills the agent table's summary and model columns.
