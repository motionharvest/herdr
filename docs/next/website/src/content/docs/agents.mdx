...
The agent table shows the current run duration while the agent is working. When it becomes idle, that duration freezes and idle age starts counting. A row that is not a linked worktree offers `Delete agent`, which ends the agent and closes the pane it was in. `Delete agent / worktree...` stays at the bottom of the same menu and is grayed out unless that agent's directory is a linked worktree. In a linked worktree, the menu also offers land onto the parent checkout's current branch. Choosing it prompts that agent to commit anything outstanding and get the checkout onto the parent branch. Land is grayed out when the worktree already shares the parent branch's commit and has nothing left to land, or when the agent is not in the worktree directory. `herdr worktree land` still runs the git. The `!` in Git Status means that checkout has uncommitted files. Deletion warns before discarding uncommitted or unlanded work. Composer-launched editing agents use the same managed-worktree path when the Worktree box next to Task is checked and their chosen directory is a repository. Clear the box to start in that directory.
