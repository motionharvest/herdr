...
`agent start --worktree` creates a linked checkout under `[worktrees].directory`, launches the agent as that checkout's workspace, and leaves the harness argv unchanged. Omit `BRANCH` for a generated `worktree/...` branch. Because it creates a workspace, `--worktree` cannot be combined with `--workspace`, `--tab`, or `--split`.
