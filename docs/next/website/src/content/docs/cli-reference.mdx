...
`worktree land` accepts a workspace ID, pane name, or unique agent name. It rebases the linked worktree onto the parent checkout's current branch, runs `[worktrees].verify`, and fast-forwards the parent. `--all` serializes every open linked worktree through the same operation.
