...
herdr worktree land [target | --all] [--json]
```

Worktrees are normal Herdr workspaces with Git checkout provenance. `worktree create` creates a Git worktree checkout, opens it as a workspace, and groups it with the parent repo workspace. Without `--path`, a relative `[worktrees].directory` creates the checkout under `<repo>/<directory>/<branch-slug>`. An absolute or `~/` directory still creates `<directory>/<repo>/<branch-slug>`.

`worktree land` accepts a workspace ID, pane name, or unique agent name. A workspace ID or `--all` lands from that space's saved worktree membership. A pane or agent name lands the linked worktree of that agent's current folder. It rebases onto the parent checkout's current branch, runs `[worktrees].verify`, and fast-forwards the parent. `--all` serializes every open linked worktree through the same operation.

`workspace close` closes Herdr state only. `worktree remove` is the explicit destructive path: without `--force`, uncommitted files or commits absent from the parent branch stop it. A successful removal deletes both the checkout and its local branch.
...
`<pane_id>` accepts a pane ID from `pane list` or the pane's assigned name, case-insensitively. An agent with a session title is named from that title: a headline that ends in a past verb becomes an `-er`/`-or` agentive on the last two content nouns (`naming-convention-explainer`), an imperative title uses the first content verb and the last two content words of its object (`yellow-banner-hider`), a noun-only title becomes `-man` or `-ist` (`workman`, `machinist`), and anything else uses the first three words as a slug (`herdr-pane-title`). Collisions get `-2`. A shell, or an agent that has not been titled yet, still gets a first name from the word list (`Olivia`, `Olivia-2` on collision). Pane IDs compact when panes close; the name stays with its pane for the life of the terminal and survives restarts, so prefer names in anything longer-lived than one command. `pane list` and `pane get` return the name as a `name` field.
...
herdr agent start <name> [--cwd PATH] [--worktree [BRANCH]] [--workspace ID] [--tab ID] [--split right|down] [--focus|--no-focus] -- <argv...>
herdr agent worktree land [target] [--json]
```

Agent targets can be terminal IDs, pane names, unique agent names, detected or reported agent labels, or legacy pane IDs. Names and labels are agent identities. Terminal IDs and legacy pane IDs are low-level escape hatches.

`agent status` prints one plain-text line per agent — name, status, and the reported session title when the agent has reported one — instead of JSON. `agent read` reads the resolved terminal stream. `agent send` writes literal text to that stream without submitting; `agent prompt` pastes the text and presses Enter, so the message arrives and submits as one. `agent get`, `agent focus`, `agent wait`, and `agent attach` require the resolved terminal to have agent identity. `agent rename` can assign that identity.

`agent worktree land` lands the linked worktree of the named agent's current folder. With no target it uses the process cwd, which is the folder the agent is running in. It does not use the space the row sits in.

`agent start --worktree` starts the agent in a linked worktree and leaves the harness argv unchanged. If `--cwd` is already a linked worktree, that checkout is reused and no new branch is created; an already-open workspace for that checkout is reused too. Otherwise Herdr creates a linked checkout under `[worktrees].directory` and launches the agent as that checkout's workspace. Omit `BRANCH` for a generated `worktree/...` branch. Because it owns placement, `--worktree` cannot be combined with `--workspace`, `--tab`, or `--split`.
...
```bash
herdr shell-init powershell
```

Prints the profile snippet PowerShell needs to report its working directory to Herdr over OSC 7. Required for `powershell.exe` started from WSL, where `/proc` cannot see the shell's location. See [PowerShell on WSL](/docs/configuration/#powershell-on-wsl).
