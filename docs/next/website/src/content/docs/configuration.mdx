...
Deleting an agent worktree is explicit. Use `Delete agent + worktree` on its agent-table row. Herdr refuses the first deletion when the checkout has uncommitted files or commits that have not landed, then shows the destructive confirmation. Confirming ends the worktree's agents and removes both the checkout and its local branch; closing a space still removes neither.
