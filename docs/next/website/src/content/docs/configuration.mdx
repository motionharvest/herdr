...
Worktree actions are available from agent-table rows. `New worktree` creates a branch and checkout and opens it as a new Herdr space. `Open worktree...` lists existing Git worktree checkouts for that repository. Composer-launched editing agents use managed worktrees automatically when their selected directory is a repository.
