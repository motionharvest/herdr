...
The land item on a linked worktree row is named for the branch currently checked out in the parent checkout, such as `Land on main`. Choosing it prompts that agent to commit outstanding work with a message that names the change, then run `herdr worktree land` from that checkout. The item stays visible but is grayed out when the worktree already shares that parent commit and has nothing left to land, or when the agent is not in that worktree directory. `herdr worktree land` still rebases onto the parent branch, runs `verify`, and fast-forwards the parent checkout. A parent checkout's row still says `Delete agent`, which ends the agent and leaves the directory alone. `Delete agent / worktree...` stays at the bottom of the menu on every agent row and is grayed out unless that agent's directory is a linked worktree. The `!` next to a directory means that checkout has uncommitted files; a dirty parent checkout blocks the fast-forward. A conflict stays in the agent worktree. Herdr serializes CLI landings for one repository so agents cannot race to move the same parent branch.
