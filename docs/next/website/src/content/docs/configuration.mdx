...
`Land on main` rebases the worktree branch onto the branch checked out in the parent workspace, runs `verify`, and fast-forwards the parent checkout. A conflict stays in the agent worktree, and a failed rebase or verification is sent back to that agent. Herdr serializes landings for one repository so agents cannot race to move the same parent branch.
