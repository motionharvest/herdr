...
The land item on a linked worktree row is named for the branch currently checked out in the parent checkout, such as `Land on main`. It rebases the worktree branch onto that parent branch, runs `verify`, and fast-forwards the parent checkout. The item stays visible but is grayed out when the worktree already shares that parent commit and has nothing left to land, or when the agent is not in that worktree directory. A parent checkout's row still says `Delete agent`, which ends the agent and leaves the directory alone. `Delete agent / worktree...` stays at the bottom of the menu on every agent row and is grayed out unless that agent's directory is a linked worktree. A status overlay opens on an available land click and stays until it is dismissed, showing the git result or the reason land stopped. The `!` next to a directory means that checkout has uncommitted files; land can commit those in the worktree, but it will not fast-forward a dirty parent checkout. A conflict stays in the agent worktree. Herdr serializes landings for one repository so agents cannot race to move the same parent branch.
