...
### Changed
- A hidden agent's name matches Summary until that row is selected, then it uses the accent. A selected name still in a space uses the default foreground. Hidden names used the accent while unselected, so on synthwave every set-down agent read as cyan.
