...
### Changed
- The linked-worktree row menu now names the parent checkout's current branch (`Land on main`, `Land on release`, or `Land on parent` if that branch cannot be read). Choosing it still commits any uncommitted files in that worktree first, then rebases and fast-forwards onto that parent branch. The row says `landing` while it runs. Success and failure are toasts, not a `config warning` banner, and a failed land leaves a red ✕ on the row until the next attempt.
