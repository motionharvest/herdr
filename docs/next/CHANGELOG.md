...
- Linux clients now show a clear clipboard image paste hint when required clipboard tools are missing (`wl-clipboard`/`wl-paste` for Wayland or `xclip` for X11).
- Linux clipboard image paste staging now normalizes image payloads to PNG so bridged image paths are AI-friendly and consistent.
...
### Breaking Changes
- Removed the separate `keys.quit` binding. Use `keys.detach`, which detaches in server mode and exits in `--no-session` mode. The default detach binding is now `ctrl+q`, and direct `ctrl+q` detach requires pressing it twice.
