...
- The linked-worktree row menu now names the parent checkout's current branch (`Land on main`, `Land on release`, or `Land on parent` if that branch cannot be read). Choosing it prompts that agent to commit and land onto that parent branch. `herdr worktree land` still commits a dirty worktree, rebases, runs verify, and fast-forwards the parent. The `!` in Git Status means that checkout has uncommitted files; a dirty parent checkout blocks the fast-forward. A failed CLI land still leaves a red ✕ on the row until the next attempt.
