...
- Agents started from the composer now work in Herdr-managed linked worktrees, regardless of harness. Herdr creates the checkout itself instead of passing Claude Code's `-w`, so Codex and every other editing harness receive the same isolation and the resulting work has one lifecycle. `herdr agent start <name> --worktree [branch] -- <argv...>` exposes the same fast path to scripts. A folder outside a repository still starts in place. If the start directory is already a linked worktree, composer start and `agent start --worktree` reuse that checkout instead of creating another, and an already-open workspace for it is reused too. `Auto` remains a router rather than creating an empty checkout for itself.
- Managed worktrees can land by agent name with `herdr worktree land <name>`, with `--all`, or from the agent table's `Land on main` action. Landing rebases onto the parent checkout's branch, runs the optional `[worktrees].verify` argv, and fast-forwards the parent; failures stay with and are reported back to the originating agent instead of leaving the parent half-merged. `[worktrees].auto_land` runs the same serialized path when an agent becomes idle and defaults off.
...
### Changed
- The agent-table row menu now says `Delete agent` for ordinary agents, which ends the agent and leaves the pane. Linked-worktree rows still offer `Delete agent / worktree...` because that action removes the checkout; a parent checkout never offers land or worktree deletion. `Land on <branch>` stays on linked-worktree rows but is grayed out when that checkout already shares the parent branch's commit and has nothing left to land.
...
- The linked-worktree row menu now names the parent checkout's current branch (`Land on main`, `Land on release`, or `Land on parent` if that branch cannot be read). Choosing it still commits any uncommitted files in that worktree first, then rebases and fast-forwards onto that parent branch. A status overlay opens immediately and stays until it is dismissed, so a click is never silent: it says `landing` while git runs, then the result or the error. The `!` next to a directory means that checkout has uncommitted files; a dirty parent checkout is named in the overlay because land cannot fast-forward it. The row still says `landing` while it runs, and a failed land still leaves a red ✕ on the row until the next attempt.
