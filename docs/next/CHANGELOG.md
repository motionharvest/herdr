...
- An agent started from the composer is shown as soon as it starts. The new pane takes focus and the keyboard leaves the band, so typing a task and sending it lands you on the agent that received it. A managed worktree already has a space of its own; an agent that would otherwise start set down is cut in beside the pane that had focus. Starting used to leave the agent hidden so the current panes stayed put, which meant a second click or drag before the work could be read.
