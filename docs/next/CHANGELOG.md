...
- The Windows terminal client now sends structured input events instead of parsed raw bytes. The default reader consumes console input records directly, so physical keys keep their layout identity (Shift+Enter, AltGr, ctrl+j, IME text, and held-key repeats survive), pastes arrive as one event, and mouse, wheel, and focus reports are forwarded faithfully. Set `HERDR_WINDOWS_INPUT_BACKEND=crossterm` to use the previous crossterm-based reader, `HERDR_WINDOWS_INPUT_PROBE` to control win32-input-mode reporting to ConPTY panes, and `HERDR_WINDOWS_INPUT_TRACE` to log every translated input event. The client restores the original console input mode on exit, and mouse capture toggles no longer break keyboard input mid-session.
