...
### Changed
- Choosing `Land on <branch>` now prompts the agent to commit outstanding work with a message that names the change, then run `herdr worktree land` from that checkout. The previous prompt named the parent branch and said to get the checkout onto it, which did not name the command that does the land.
