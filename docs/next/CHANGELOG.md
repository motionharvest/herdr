...
### Added
- Claude Code started from the composer now works in a git worktree of its own. The row passes `-w`, which is Claude Code's own request for a fresh worktree branched from the folder it was started in, so two agents pointed at one repository no longer edit one checkout and overwrite each other's work. The flag is passed only where the folder is inside a repository — Claude Code refuses to start at all when asked to branch from a repository that is not there — and a folder outside one still starts the agent where it is. Starting an agent again from its row does not branch again: the worktree flag comes off with the task, because a second empty checkout is not where the first one's work is. `Auto` is unchanged, since the agent it starts reads transcripts and hands the message to whichever agent already owns the work rather than editing anything itself.
