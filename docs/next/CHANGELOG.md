...
### Changed
- Choosing `Land on <branch>` now prompts the agent to commit outstanding work, then run `herdr worktree land`. The previous prompt named the parent branch and said to get the checkout onto it, which did not name the command that does the land.
