...
### Fixed
- Peeking a set-down agent now outlines that pane in the accent color and clears the selected mark on the docked agent in the spaces sidebar. The overlay used the focus color, so on synthwave it looked pink like a normal focused pane instead of cyan, and the agent underneath stayed highlighted even though the keyboard was on the peek.
- `Refresh Summary` with `refresh summary with grok` on now reads the sentence out of Grok's JSON envelope. `--json-schema` prints `{ "text": "{\"summary\":\"…\"}", … }`, and the command used the first line of that pretty-printed object, so Summary became `{` and clicking the item again looked like it did nothing. If the envelope has no usable sentence, the command falls back to the session-log prompt the way it does when Grok fails.
...
- A Grok or Codex agent's Summary fills from the first typed prompt and then stays. Follow-up prompts used to replace it, so a line like `Land this on parent` wiped the purpose of the session. The headline is still the part of the prompt that asks for work, not the whole message. `Refresh Summary` on the row menu takes the latest prompt when you want that instead, or a 5–8 word Grok sentence when Settings → experiments → `refresh summary with grok` is on. `generated_title` still fills a Grok row when no prompt has been parsed yet.
...
- An agent's assigned name is derived from its session title. A headline that ends in a past verb uses that verb as an `-er` or `-or` agentive and the last two content nouns before it as the head compound (`Clever Agent Naming Convention Explained` → `naming-convention-explainer`). An imperative title uses the first content verb and the last two content words of its object (`Hide yellow bad weather banner from pages` → `yellow-banner-hider`, `Add Playbill link to main navigation` → `playbill-link-adder`, `Add Donate Auction Survey nav links` → `nav-links-adder`). A title with no content verb uses `-man` on a one-syllable noun (`workman`) or `-ist` on a longer one (`machinist`). If that does not yield a name, the first three words of the title are used as a slug. The name is frozen on first success. Until a title exists, and for shells that never get one, the first-name word list is still used. Collisions still get `-2`. A manual pane or agent rename still wins.
