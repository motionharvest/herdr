...
- An agent's assigned name is derived from its session title. A content verb becomes an `-er` or `-or` agentive and pairs with a noun when one exists (`Commit work and land herdr worktree` → `work-committer`). A title with no content verb uses `-man` on a one-syllable noun (`workman`) or `-ist` on a longer one (`machinist`). Copulas are skipped. If that does not yield a name, the first three words of the title are used as a slug. The name is frozen on first success. Until a title exists, and for shells that never get one, the first-name word list is still used. Collisions still get `-2`. A manual pane or agent rename still wins. The English tokenizer binaries come from nlprule and are licensed LGPLv2.1.
...
- Clicking a hidden agent now peeks that agent in FOCUS over the current layout. Double-clicking used to, and a second click used to leave the peek, which hid the agent you were looking at. The splits underneath still do not change, so a shell running `npm run dev` keeps running. BACK or HIDE leaves the peek. Clicking the same row again keeps it showing. Dragging still places the agent: an edge splits, the middle replaces. Double-clicking the name still renames. A visible agent's row still focuses its pane.
