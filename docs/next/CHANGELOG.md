...
### Added
- Right-clicking an agent in the table or the spaces sidebar now offers `Refresh Summary`. It reads the current prompt into the Summary column the way the automatic update used to, but only when you ask.
...
- A Grok or Codex agent's Summary fills from the first typed prompt and then stays. Follow-up prompts used to replace it, so a line like `Land this on parent` wiped the purpose of the session. The headline is still the part of the prompt that asks for work, not the whole message. `Refresh Summary` on the row menu takes the latest prompt when you want that instead. `generated_title` still fills a Grok row when no prompt has been parsed yet.
